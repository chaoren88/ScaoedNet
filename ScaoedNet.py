...
class DE_Net(nn.Module):
    def __init__(self, nf = 64, nf_2=64, input_para=1,num_blocks= 5):
        ...
        self.head_Q = BasicConv(64, nf_2, 3, stride=1, padding=(3 - 1) // 2, relu=False)
...
class RE_Net(nn.Module):
    def __init__(self):
        ...
        self.head_E = BasicConv(n_feats, n_feats, 3, stride=1, padding=(3 - 1) // 2, relu=False)


        self.head_noisy = BasicConv(inp_chans*4, n_feats, 3, stride=1, padding=(3 - 1) // 2, relu=False)
        self.head_z = BasicConv(n_feats, n_feats, 3, stride=1, padding=(3 - 1) // 2, relu=False)
...
class DN(nn.Module):
    ...
    def forward(self, noisyImage):
        ...
        for i in range(self.T):
            ...
            outs.append(u)
        return outs, self.m_up(self.tail(z))
